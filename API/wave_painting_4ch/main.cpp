...
#define XMIN 20							//描画領域のx座標の最小値
#define XMAX 580						//描画領域のx座標の最大値
...
UINT WINAPI TFunc(LPVOID thParam)
{
	...
	char buf[64];
	int t;								//測定時間
	double data[4], prevdata[4];		//データ
	DWORD DNum = 0, beforeTime;
	DWORD nowTime, progress, idealTime;
	int time = 0;
	HDC	hdc1, hdc2, hdc3, hdc4;				//デバイスコンテキストのハンドル
	...
	fgets(buf, sizeof(buf), fp); //最初の1行を飛ばす
...
void PaintAxis(HWND hWnd, COLORREF color, COLORREF colorPen) {
	...
	********************************/
	...
	LineTo(hdc, XMIN, 252);											//y軸描画


	//ここまで
	SetTextColor(hdc, RGB(255, 255, 255));
	SetBkMode(hdc, TRANSPARENT);
	TextOut(hdc, 290, 250, TEXT("Time [s]"), 8);		//テキスト描画
